...
- **Something found** — still propose the `allowWrite` addition (the directory has a legitimate reason to need writes, e.g. it's a git repo the user pushes from), but carve out each specific secret-bearing path with `sandbox.filesystem.denyRead`, scoped as narrowly as possible — never the whole directory.
- **`denyRead` vs `denyWrite`, pick based on what generates the file:**
  - **Static secret, never regenerated by anything the agent might run** (an SSH key, a checked-in cert) — add both `denyRead` and `denyWrite`. Precedent: `~/.lima/_config/user` (Lima's own SSH private key) is denied both ways.
  - **Output of a generator tool the agent might legitimately invoke** (a cert-issuing script, a key-generation CLI) — `denyRead` only. `denyWrite` would break the tool's ability to write its own output. Precedent: `tools/certool/output/` (TLS keys regenerated by a certool script) is `denyRead`-only — the agent can still run certool to produce new keys, just can't read the resulting private key material back into context.
- Always show the reasoning for the read-vs-write choice in the proposal (Step 4) — never propose `allowWrite` without having run this scan.
...
- [ ] Any secret-bearing file found has a `denyRead` entry (plus `denyWrite` only if nothing legitimately regenerates it) in the same proposal
